...
const apiList = [PaginateResults, PaginateNoResults, PaginateLoading, PaginateFooter, PaginateNav]
...
class Paginate extends React.Component {
  ...
  componentDidMount() {
    const { page, resultsPerPage } = this.normalizeInput(this.props.page, this.props.resultsPerPage)
    this.setState({ page, resultsPerPage }, this.fetchResults)
  }

  componentWillReceiveProps(nextProps) {
    const { page, resultsPerPage } = this.normalizeInput(nextProps.page, nextProps.resultsPerPage)
  ...
  normalizeInput(page, resultsPerPage) {
    page = this.validateInput(page)
    resultsPerPage = this.validateInput(resultsPerPage)
    ...
    this.props.api(this.state.page, this.state.resultsPerPage)
      .then(response => {
        if (!_.isPlainObject(response)) {
          console.error('API response for `react-paginator` is not an object . Received:', response)
          ...
          if (!Array.isArray(results) || !Number.isInteger(parseInt(totalResults))) {
            console.error('API response for `react-paginator` is malformed. Received:', response)
            return
          }
  ...
  render() {
    ...
    if (Array.isArray(this.state.results)) {
      const seek = this.state.results.length > 0 ? [PaginateResults, PaginateFooter, PaginateNav] : [PaginateNoResults]
      clonedChildren = this.iterateChildren(this.props.children, seek)

    // If no results haven't returned, return the loading component
    } else {
      clonedChildren = React.Children.map(this.props.children, child => {
        if (child.type === PaginateLoading) {
    ...
    return <div className={classnames('react-paginator', this.props.className)}>{clonedChildren || null}</div>
  }
}
